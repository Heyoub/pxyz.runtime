...
Component Details
Runtime (Native) Language: Rust <br> Purpose: Development, testing, and debugging <br> Lines of Code (approx.): ~1500
Runtime (WASM) Language: WAT <br> Purpose: Production deployment and security auditing <br> Lines of Code (approx.): ~500
...
Its primary child elements are:
...
The following table details all available predicate expression elements.
...
3. Host Imports: The runtime is completely sandboxed and communicates with the outside world exclusively through a minimal, well-defined set of imported functions provided by the host. The io_call function is the single, critical gateway for all external side effects.
...
1. Safety Limits: To prevent unbounded execution and resource exhaustion, the runtime enforces strict, non-configurable safety limits. Tripping any of these limits immediately terminates the traversal.
...
SYN005 At least one entry point is defined.
...
SEM004 No cycles in graph (must be a DAG).
SEM005 All nodes are reachable from an entry point.
...
Code Check Description
PRAG001 LLM → Irreversible action paths require a validation gate.
PRAG002 Write operations should have defined error-handling branches.
PRAG003 Irreversible actions require a human-in-the-loop path.
PRAG004 Irreversible actions require inputs that have been explicitly confirmed.
PRAG005 Data marked as 'quarantined' cannot escape to an external I/O operation.
...
* --format <FORMAT>: Sets the output format. Options are:
